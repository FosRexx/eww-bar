...
fn main() -> Result<()> {
    ...
    let mon_clone = Arc::clone(&mon);
    event_listener.add_layer_opened_handler(move |_| {
        output(&mon_clone);
    });
    let mon_clone = Arc::clone(&mon);
    ...
    let mon_clone = Arc::clone(&mon);
    event_listener.add_urgent_state_changed_handler(move |_| {
        output(&mon_clone);
    });
    let mon_clone = Arc::clone(&mon);
    event_listener.add_window_title_changed_handler(move |_| {
        output(&mon_clone);
    });
